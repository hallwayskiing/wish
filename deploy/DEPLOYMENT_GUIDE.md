...
- `src/worker.ts`: Worker API 路由入口。
- `src/routes.ts`: API 路由匹配、鉴权与请求分发。
- `src/wishes.ts`: 愿望生成、保存、查询与助愿逻辑。
- `src/model.ts`: Gemini 模型请求与响应解析。
- `src/prompt.ts`: 模型 Prompt。
